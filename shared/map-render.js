            ...
            const content = `
                <div class="font-sans">
                    <div class="border-b pb-1 mb-2">
                        ...
                        <div class="flex justify-between items-center mt-1"><span class="text-[10px] text-gray-500">${item.pdr}</span><span class="px-2 rounded text-[9px] font-bold ${item.fatto ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800'}">${item.fatto ? 'FATTO' + (item.data_fatto ? ' ' + item.data_fatto : '') : 'DA FARE'}</span></div>
                    </div>
